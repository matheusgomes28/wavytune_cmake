...
Matrix<std::complex<double>> get_multiplier(std::size_t N)
{
	...
	const std::complex<double> W = std::exp(-(2.0 * MATH_PI / static_cast<double>(N)) * 1*i);
...
Matrix<std::complex<double>> fft_helper(const signal& input, const std::vector<FFT_PARTITION>& partition_order)
{
	...
	if ((input.size() / half_divisor) > 8)
	{
		...
		basis.reserve(basis_size);
		const std::complex<double> W = std::exp(-(2.0 * MATH_PI / static_cast<double>(N)) * std::complex<double>(0,1));
		for (std::size_t i = 0; i < basis_size; ++i)
		{
			basis.push_back(std::pow(W, i));
		}

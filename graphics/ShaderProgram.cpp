...
namespace
{
	void compile_shader(wt::Shader& shader)
	{
		shader.compile();
	}

	void compile_shaders_if_necessary(wt::ShaderPointer& fs, wt::ShaderPointer& gs, wt::ShaderPointer& vs)
	{
	...
	{
		std::cout << "Fragment Shader: " << fs->get_compilation_message() << std::endl;
		if (gs)
		{
			std::cout << "Geometry Shader: " << gs->get_compilation_message() << std::endl;
		}
		std::cout << "Vertex Shader: " << vs->get_compilation_message() << std::endl;
	}

	void attach_shaders(unsigned int program_id, unsigned int fs_id, unsigned int vs_id)
	{
	...
	void attach_shaders(unsigned int program_id, unsigned int fs_id, unsigned int gs_id, unsigned int vs_id)
	{
	...
	void detach_shaders(unsigned int program_id, unsigned int fs_id, unsigned int vs_id)
	{
		Expects(program_id != 0);
		Expects(fs_id != 0);
		Expects(vs_id != 0);
	...
	void detach_shaders(unsigned int program_id, unsigned int fs_id, unsigned int gs_id, unsigned int vs_id)
	{
	...
	unsigned int generate_program_address()
	{
...
ShaderProgram::ShaderProgram(ShaderPointer fragment_shader, ShaderPointer vertex_shader)
	: _address(0)
	, _fs{std::move(fragment_shader)}
	, _vs{std::move(vertex_shader)}
{
}

ShaderProgram::ShaderProgram(ShaderPointer fragment_shader, ShaderPointer geometry_shader, ShaderPointer vertex_shader)
	: _address(0)
	, _fs{std::move(fragment_shader)}
	, _gs{std::move(geometry_shader)}
	, _vs{std::move(vertex_shader)}
{
}
...
void ShaderProgram::compile_and_link()
{
	...
	if (_gs)
	{
		attach_shaders(_address, _fs->get_address(), _gs->get_address(), _vs->get_address());
		link_program(_address);
		detach_shaders(_address, _fs->get_address(), _gs->get_address(), _vs->get_address());
	}
	else
	{

...
namespace wt::matrix
{
  ...
  template <> class MatrixTraits<int>
  {
  public:
    static constexpr int default_val = 0;
  };

  template <> class MatrixTraits<float>
  {
  public:
    static constexpr float default_val = 0;
  };

  template <> class MatrixTraits<double>
  {
  public:
    static constexpr double default_val = 0;
  };

  template <typename T> class MatrixTraits<std::complex<T>>
  {
  public:
